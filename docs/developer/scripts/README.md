...
3. Occasionally a ssh command will fail because of connection problems. If this happent the scrept will fail with "unexpected EOF". If this happens run the script with ```-d``` to clean up resources. Then re-run your original command.
